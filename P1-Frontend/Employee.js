...
document.getElementById('submitButton').addEventListener('click', newReimb);
        ...
        for(let reimb of data) { //for every avenger in the data variable/object
            ...
            row.appendChild(cell2);
                        ...
                        if(reimb.reimb_resolved != null){
                            ...
                            cell4.innerHTML = reimb.reimb_submitted;
     ...
     //       cell3.innerHTML = reimb.reimb_submitted;
       //     row.appendChild(cell3);
            ...
            if(reimb.reimb_resolver != null){
                ...
                cell7.innerHTML = reimb.reimb_submitted;
...
async function newReimb() { //async returns a promise (which fetch returns)



    let newTicketAmount = document.getElementById("newAmount").value;
    let newTicketDescription = document.getElementById("newDescription").value;
    let newTicketAuthor = document.getElementById("newAuthor").value;
    let newTicketStatus = 1;
    let newTicketType = document.getElementById("newType").value;

    let newTicket = {
reimb_amount:newTicketAmount,
reimb_description:newTicketDescription,
reimb_author:newTicketAuthor,
reimb_status_id:newTicketStatus,
reimb_type_id:newTicketType

    }

    //we will send a fetch request to get our avenger data
    //await makes the async functions wait until the promise returns with the fetched data
    let response = await fetch(url + 'addTicket', {
method: "POST",
